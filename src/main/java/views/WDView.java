...
public class WDView implements Observer {
  ...
  public void update(Observable o, Object arg) {
    ...
    if (!excludedContexts.contains(GameMap.getGameMap().getCurrentContext().name())) {
      ...
      for (Map.Entry<String, Integer> entry : worldDomination.getTotalArmiesOwnedByPlayer().entrySet()) {
      ...
      for (Map.Entry<String, Double> entry : worldDomination.getPlayerOwnerShipPercentage().entrySet()) {
        String key = entry.getKey();
      ...
      for (Map.Entry<String, ArrayList<String>> entry : worldDomination.getContinentsOwnedByPlayers().entrySet()) {
        String key = entry.getKey();
      ...
      service.submit(() -> appInstance.updateContinentControlLabel(playerContinentOwnershipLabelString.toString()));
    }
  }
}
