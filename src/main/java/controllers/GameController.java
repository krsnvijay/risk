...
public class GameController {
  ...
  public static boolean reinforce(GameMap gameMap, String command) {
    ...
    if (armiesToPlace < 1) {
      display("numOfArmy should be > 0", false);
      return false;
    }
  ...
  public static boolean fortify(GameMap gameMap, String command) {
    if (command.contains("-none")) {
      display(
          String.format("%s chose not to fortify", gameMap.getCurrentPlayer().getPlayerName()),
          true);
      changeToNextPhase(gameMap);
      return true;
    }
    ...
    if (result) {
      display(
          String.format("Fortified %s with %d army(s) from %s", toCountry, armyToMove, fromCountry),
          true);
      changeToNextPhase(gameMap);
    } else {
      display(
